...
void solve(){
    string s;
    cin >> s;
    vector<int> position_index, cnt;
    position_index.assign(s.size(), 0);
    for (int i = 1, j = 0; i < (int)s.size(); i++) {
        while (j && s[i] != s[j]) j = position_index[j - 1];
        if (s[i] == s[j]) j++;
        position_index[i] = j;
    }
    ...
    int n = s.size();
    cnt.assign(n + 1, 0);
    for (int i = 0; i < n; i++) cnt[position_index[i]]++;
    for (int i = n; i > 0; i--) cnt[position_index[i - 1]] += cnt[i];
    for (int i = 1; i <= n; i++) cnt[i]++;

    vector<pair<int, int>> resp;
    for (int x = n; x; x = position_index[x - 1]) resp.push_back({x, cnt[x]});
    reverse(resp.begin(), resp.end());

    cout << resp.size() << '\n';
    for (auto [l, c] : resp) cout << l << ' ' << c << '\n';
}
