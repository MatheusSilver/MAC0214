...
void solve(){
    ...
    string s, p;
    cin >> s >> p;
    position_index.assign(p.size(), 0);
    for (int i = 1, j = 0; i < (int)p.size(); i++) {
        while (j && p[i] != p[j]) j = position_index[j - 1];
        if (p[i] == p[j]) j++;
        position_index[i] = j;
    }
    ...
    for (int i = 0, j = 0; i < (int)s.size(); i++) {
        while (j && s[i] != p[j]) j = position_index[j - 1];
        if (s[i] == p[j]) j++;
        if (j == (int)p.size()) {
            pos.push_back(i - (int)p.size() + 1);
            j = position_index[j - 1];
        }
    }
