...
void solve(){
    ...
    ll neighbor = 0;
    for (int i = 1; i <= d; i++) {
        neighbor += weights[(n - i) % n];
        neighbor += weights[i];
    }

    ll happy = 0;
    for (int person = 0; person < n; person++) {
        ll relative = 2LL * d * weights[person] - neighbor;
        if (relative > 0) happy += relative;

        neighbor -= weights[(person - d + n) % n];
        neighbor += weights[(person + d + 1) % n];
        neighbor -= weights[(person + 1) % n];
        neighbor += weights[(person + 0) % n];
    }
    cout << happy << "\n";
}
