...
void solve(){
    int n;
    cin >> n;
    vector<int> parent(n + 1), h(n + 1, 0), second(n + 1, 0);
    for (int i = 2; i <= n; i++) cin >> parent[i];
    for (int i = n; i >= 2; i--) {
        int par = parent[i], dc = h[i] + 1;
        if (dc > h[par]) second[par] = h[par], h[par] = dc;
        else if (dc > second[par]) second[par] = dc;
    }
    ll answer = n;
    for (int v = 1; v <= n; v++) answer += second[v];
