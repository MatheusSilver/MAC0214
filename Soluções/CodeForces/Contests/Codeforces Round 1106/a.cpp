...
void solve(){
    ...
    for (int i = 0; i < len; i++) cin >> source[i];
    for (int i = 0; i < len; i++) cin >> target[i];

    int sourceSum = 0, targetSum = 0;
    for (int i = 0; i < len; i++) {
        sourceSum = sourceSum + source[i];
        targetSum = targetSum + target[i];
    }
    int decrementsNeeded = sourceSum - targetSum;
    ...
    int sortedSource[105], sortedTarget[105];
    for (int i = 0; i < len; i++) {
        sortedSource[i] = source[i];
        sortedTarget[i] = target[i];
    }
    sort(sortedSource, sortedSource + len);
    sort(sortedTarget, sortedTarget + len);
    ...
    if (noReod) answer = decrementsNeeded;

    if (withReod) {
        int cReod = reodCost + decrementsNeeded;
        if (answer == -1 || cReod < answer) answer = cReod;
    }
