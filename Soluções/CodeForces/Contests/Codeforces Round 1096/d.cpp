...
void solve(){
    int n,m,ans=0;cin>>n;m=2*n;
    vector<int>a(m);for(int i=0;i<m;i++)cin>>a[i];
    for(int s=0;s<2;s++)for(int c=0;c+s<m;c++){
        int l=c,r=c+s,mx=0;vector<char>h(n+1);
        while(l>=0&&r<m&&a[l]==a[r]){
            if(0<=a[l]&&a[l]<=n){h[a[l]]=1;while(mx<=n&&h[mx])mx++;}
            if(l!=r&&0<=a[r]&&a[r]<=n){h[a[r]]=1;while(mx<=n&&h[mx])mx++;}
            if(mx>ans)ans=mx;
            l--;r++;
        }
    }
