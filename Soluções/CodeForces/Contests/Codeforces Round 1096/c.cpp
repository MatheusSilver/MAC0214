...
void solve(){
    int n;
    cin>>n;
    static int g[4][maxn]; int c[4]; c[0]=c[1]=c[2]=c[3]=0;
    for(int i=0;i<n;i++){int v;cin>>v; if(v%6==0) g[0][c[0]++]=v; else if(v%2==0) g[1][c[1]++]=v; else if(v%3==0) g[3][c[3]++]=v; else g[2][c[2]++]=v;}
    bool d=0; for(int k=0;k<4;k++) for(int i=0;i<c[k];i++){ if(d) cout<<' '; cout<<g[k][i]; d=1; } 
    cout<<'\n';
}
