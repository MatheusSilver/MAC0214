/* Usava isso na matéria de Desafios, queria evitar o uso mas fiquei com preguiça */
/* Ainda mais depois de ver o coiso do mod */
...
void solve(){
    ...
    for (int i = 1; i <= n; i++){
        for (int j = 1; j <= n; j++){
            dp[i][j] = (grid[i-1][j-1] == '.' ? dp[i-1][j]+dp[i][j-1] : 0)%mod;
        }   
    }
