...
void solve(){
    ...
    double resposta = 0.0;
    double esperanca = 1.0;

    int numDigitos = s.size();

    for (int i = 0; i < numDigitos; ++i) {
        int digitoAtual = s[i] - '0'; // BURRO ESQUECEU TECPROG
        int digitosRestantes = numDigitos - 1 - i;

        resposta += digitoAtual * esperanca * (0.9 * digitosRestantes + 1.0);
        esperanca *= 0.1;
    }

    cout << fixed << setprecision(8) << resposta << endl;
}
