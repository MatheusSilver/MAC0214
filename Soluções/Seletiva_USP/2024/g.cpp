...
void solve() {
    ...
    for (int tel = 0; tel <= n; tel++) dp[n][tel] = 0;

    for (int cur = n - 1; cur >= 1; cur--) {
        for (int tel = 0; tel <= n; tel++) {
            ll walk = dp[cur + 1][tel] + abs(pos[cur + 1] - pos[cur]);

            ll teleport = dp[cur + 1][cur] + abs(pos[cur + 1] - pos[tel]);

            dp[cur][tel] = min(walk, teleport);
        }
    }

    cout << abs(pos[1]) + dp[1][0] << '\n';
}
