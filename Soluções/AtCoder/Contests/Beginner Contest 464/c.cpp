...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        cin >> A[i] >> D[i] >> B[i];
        hashMap[A[i]]++;
        change[D[i] - 1].push_back(i);
    }

    int distinctColor = (int)hashMap.size();

    for (int j = 0; j < M; j++) {
        for (int bird : change[j]) {
            int oldColor = A[bird];
            int newColor = B[bird];
            ...
            hashMap[oldColor]--;
            distinctColor -= hashMap[oldColor] == 0;
            
            if (hashMap[oldColor] == 0) {
                hashMap.erase(oldColor);
            }
            
            hashMap[newColor]++;
            distinctColor += hashMap[newColor] == 1;
