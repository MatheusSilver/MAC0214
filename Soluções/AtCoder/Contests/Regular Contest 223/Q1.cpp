...
int n;
ll capacityLimit;
ll weight[65], value[65];
ll prefixWeight[65], prefixValue[65];

ll bestValue(int idx, ll cap) {
    if (cap >= prefixWeight[idx]) return prefixValue[idx];

    ll answer = bestValue(idx - 1, cap);
    if (cap >= weight[idx]) {
        ll taken = value[idx] + bestValue(idx - 1, cap - weight[idx]);
...
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        prefixValue[i] = prefixValue[i - 1] + value[i];
    }
    
    cout << bestValue(n, capacityLimit) << "\n";
}
